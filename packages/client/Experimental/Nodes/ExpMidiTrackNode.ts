...
import {getExpMidiTrackNodeView} from './ExpMidiTrackNodeView'
import {SeqReadEvent, SeqTimelineTrack, makeSeqTimelineTrack, seqTimelineTrackReader} from '@corgifm/common/SeqStuff'
import {minZoomX, maxZoomX, maxZoomY, minZoomY, maxPan, minPan} from '@corgifm/common/BetterConstants'
import {expMidiTrackMainTrackParamId} from '@corgifm/common/common-constants'
import {logger} from '@corgifm/common/logger'

export class ExpMidiTrackNode extends CorgiNode {
	...
	public constructor(corgiNodeArgs: CorgiNodeArgs) {
		...
		this._midiTimelineTrackParam = new ExpReferenceParam(
			expMidiTrackMainTrackParamId, makeSeqTimelineTrack() as SeqTimelineTrack, 'midiTimelineTrack')
		this._referenceParams = arrayToESIdKeyMap([this._midiTimelineTrackParam] as unknown as ExpReferenceParam[])
	...
	public render = () => {
		return this.getDebugView(getExpMidiTrackNodeView())
	}
	...
	public onTick(currentGlobalTime: number, maxReadAheadSeconds: number) {
		...
		const events = seqTimelineTrackReader(readRangeBeats, this._midiTimelineTrackParam.value.current)
		...
		this._currentClipPositionBeats = toBeats(currentSongTimeSeconds) % this._midiTimelineTrackParam.value.current.loopEndBeat
	}
}
